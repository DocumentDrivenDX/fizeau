...
from scripts.benchmark.harness_adapters.ddx_agent import Agent as DDXAgent
...
class BaseCalibrationAdapterTest(unittest.TestCase):
    ...
    def test_ddx_agent_command_maps_profile_to_env(self) -> None:
        agent = DDXAgent()
        spec = agent.command(self.profile, "solve", "/tmp/task")
        self.assertEqual(spec.argv[:4], ["/installed-agent/ddx-agent", "--json", "--preset", "benchmark"])
    ...
    def test_ddx_agent_telemetry_reads_session_end_usage(self) -> None:
        stream = '{"type":"session.end","data":{"usage":{"input":10,"output":7}}}'
        telemetry = DDXAgent().parse_telemetry(stream)
