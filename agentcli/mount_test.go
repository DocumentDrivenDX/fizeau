...
func TestNeedsLegacyPassthrough(t *testing.T) {
	tests := []struct {
		...
		{name: "native route status command", args: []string{"--work-dir", "/tmp/work", "route-status", "--policy", "smart"}, want: false},
		{name: "native policies command", args: []string{"--work-dir", "/tmp/work", "policies"}, want: false},
		{name: "native harnesses command", args: []string{"--work-dir", "/tmp/work", "harnesses"}, want: false},
