...
func TestRoutingPolicyInvariants(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			wantRejected: map[string]FilterReason{
				"agent/local": FilterReasonScoredBelowTop,
				"agent/paid":  FilterReasonScoredBelowTop,
		...
		},
		{
			...
			wantRejected: map[string]FilterReason{
				"agent/local": FilterReasonBelowMinPower,
			},
		},
		{
		...
		},
		{
			...
			wantModel:       "paid-strong",
			wantRejected: map[string]FilterReason{
				"agent/paid@primary": FilterReasonPinMismatch,
			},
		},
		{
