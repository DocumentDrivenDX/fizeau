#!/usr/bin/env bash
# Demo: fiz uses bash tool to explore a project.
set -euo pipefail
...
echo "$ fiz -p 'List all Go files in this project and summarize the package structure'"
echo ""
./fiz -p "Use bash to find all .go files in this project, then summarize the package structure. Be concise." \
  --work-dir "$WORK" 2>&1
