...
| Qwen3.6-27B (frontier reasoning lane) | All three Qwen provider rows on the providers page | A `fiz-harness-codex-qwen-3-6-27b` lane (Codex CLI configured against an OpenAI-compat Qwen endpoint) | The only Qwen rows today use fiz's built-in loop. A second harness on the same Qwen weights would let the providers-page numbers be re-read as model-loss vs harness-loss. |
