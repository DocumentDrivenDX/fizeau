...
|---|---|---:|---|
| `fiz-openrouter-qwen3-6-27b` | OpenRouter `reasoning` object | yes, direct usage tokens | 2026-05-12 probe emitted nonzero reasoning on named rows; the verification report records direct `usage.completion_tokens_details.reasoning_tokens`. |
...
`reasoning_levels: [high, max]`, `reasoning_default: high`, and no client token
budget table. This matches ds4 `/props` and the observed flat
`reasoning_effort` request contract.
