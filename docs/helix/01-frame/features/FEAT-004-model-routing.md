...
**Feature ID**: FEAT-004
**Status**: Draft
**Priority**: P0 (named providers), P1 (shared catalog), P1 (power-based routing)
...
- **Route candidate** — one discovered `(harness, provider, endpoint, model)`
  option after joining live inventory with catalog metadata
...
8. The catalog represents:
   ...
   - optional aliases or human power-band names
...
18. Catalog defaults are tiered by expected capability and cost. Human aliases
    such as `cheap`, `fast`, `standard`, or `smart` may map to power ranges and
    reasoning defaults, but automated callers use numeric `MinPower` and
    `MaxPower` rather than profile names.
...
23. DDX Agent builds the candidate set by enumerating available harnesses,
    native providers, provider endpoints, and discovered model IDs, then joining
    that live inventory with catalog metadata.
...
25. Routing filters candidates by hard caller constraints first:
    ...
    - `Provider` means only that provider may be used.
...
27. Automatic routing excludes inactive, deprecated, exact-pin-only, and
    unknown-power models unless the caller explicitly pins that model and live
    discovery confirms it is available.
28. The router ranks survivors by model power, provider/deployment placement,
    effective marginal cost, prepaid quota, availability, context/capability,
    and observed speed/latency.
29. When unpinned local/free LLMs are available and satisfy requested power,
...
30. `agent.Run()` still receives one concrete `Provider` per attempt. `Execute`
    selects and dispatches the top candidate once.
31. The selected concrete harness, provider, endpoint, model, requested model
    input, resolved model reference, power bounds, and score components are
    recorded in the result/session artifacts.
32. Existing `backends`, `default_backend`, `--backend`, and user-authored
...
33. DDX Agent may track recent provider/endpoint/model availability failures
    and temporarily back off unhealthy candidates using a bounded cooldown
    window.
34. Availability feedback is keyed at least by `(harness, provider, endpoint,
    model)` so one bad endpoint or model does not poison a whole provider,
    family, or power band.
35. `Execute` does not try a second candidate after dispatching the selected
    candidate. It returns attempted-route identity and failure class so callers
    can decide whether to issue a new request.
36. Prompt-shape, tool-schema, semantic task failures, test failures, review
    blocks, or low-quality output do not trigger agent-owned retry. Those are
    caller-owned retry/escalation signals.
37. Callers continue to pass model intent (`MinPower`, `MaxPower`, `model_ref`,
    or exact pin) into the embedded harness. Callers must not duplicate inner
    provider-selection logic.

#### Phase 2C (P1): Power Aliases, Placement, and Quota-Aware Scoring

38. Human-facing aliases (`cheap`, `fast`, `standard`, `smart`, `local`,
    `offline`, `air-gapped`) may map to numeric power bounds and placement
    constraints. They are UI conveniences, not the ordering contract.
...
- **Updateability**: rapidly changing model policy/data can be refreshed via an
  ...
  release.
- **Compatibility**: named-provider configuration remains valid. Legacy
  route-table compatibility, when present, is migration-only and not the target
  architecture.
...
- **Unknown model reference**: catalog resolution returns an error before the
  run.
- **Unsatisfied exact model/provider/harness pin**: route resolution returns a
  no-candidate error with rejected candidates and reasons; it does not choose a
  broader model, provider, or harness.
...
- **Selected provider not reachable**:
  return an attempted-route error containing harness, provider, endpoint,
  model, and failure class. The agent does not try another candidate in that
  request.
...
- Power-based routing selects one candidate deterministically and records the
  actual harness/provider/endpoint/model used.
...
| AC-FEAT-004-04 | An explicit concrete `--model`, `--provider`, or `--harness` is a hard constraint. If it cannot be satisfied, routing returns detailed no-candidate evidence and never substitutes a different model/provider/harness. | `go test ./internal/routing ./cmd/ddx-agent ./...` |
...
| AC-FEAT-004-10 | The starter shared catalog publishes concrete model entries with 1..10 power, provider/deployment class, power provenance, costs, context, benchmark inputs, and surface projections. Human aliases may remain as optional compatibility macros over numeric bounds. | `go test ./internal/modelcatalog ./internal/config ./cmd/ddx-agent ./...` |
