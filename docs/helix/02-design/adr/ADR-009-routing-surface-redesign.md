...
- pay-per-token providers are excluded from unpinned/default routing unless
  policy data or user config opts them in.
...
Pay-per-token providers are default-deny for unpinned/default routing:
`include_by_default: false` unless the catalog or user config opts the provider
in. Fixed/local and subscription/account surfaces may be included by default
when the catalog marks them as such.
...
Pins override `include_by_default`: an explicitly pinned pay-per-token provider
or exact model may be considered even when that provider is default-deny for
automatic routing.
...
In short: a pin overrides `include_by_default` but NOT `require[]`.
...
- Default-deny pay-per-token behavior is explainable from provider billing and
  `include_by_default`, reducing accidental spend.
...
- pay-per-token default routing -> set `include_by_default: true` explicitly
